...
from models.model_retrieval import SPMA
...
def set_trainable(model):
    ...
    for name, module in model.named_modules():
        if ('BiShareAdapter' in name) or ('mmadapter' in name)  or ('MMadapter' in name) or ('project_layer' in name) or ('static_prompts' in name):
            module.train()
            for param in module.parameters():
...
def train(model, data_loader, optimizer, tokenizer,epoch, device, scheduler, config,trainable_params):
    ...
    for i, (image, text, idx, label) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        ## choose the loss
        if config['use_affil_loss']:
            loss_contr, loss_affil = model(image, text_input, idx=idx, label=label)
            loss = config['center_factor'] * loss_contr +  loss_affil
            # loss = 1 * loss_contr +  0.5 * loss_affil
        elif config['use_triplet_loss']:
...
def main(args, config):
    ...
    model = SPMA(config=config)
    trainable_params = set_trainable(model)
    sum_trainable = count_trainable_parameters(model)
    print(sum_trainable)
    ...
    if args.evaluate:
        ...
        for epoch in range(0, max_epoch):
            ...
            train_stats = train(model, train_loader, optimizer, tokenizer, epoch, device, lr_scheduler, config,trainable_params)
            score_test_i2t, score_test_t2i = evaluation(model_without_ddp, test_loader, tokenizer, device, config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str, required=False, default='-1')
