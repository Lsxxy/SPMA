...
def tree_scanning_algorithm(self, input_states, contex_len, cache_params):
    ...
    with torch.no_grad():
        ...
        if contex_len > 2:
            ...
            data1 = torch.index_select(feature_in,2,pairs[:,0])     #这里的feature_in已经是hidden_states*离散化B所得到的，相当于是SSM特征了
            data2 = torch.index_select(feature_in,2,pairs[:,1])     #index_select函数，第一个参数代表从哪里提取数据，这里是SSM特征，第二个参数代表从哪个维度上进行提取，这里为2，为len，即时间步长或者序列步长，第三个参数代表提取哪些数据的索引，
            # import pdb;pdb.set_trace()                             这里pairs[:,0]代表所有边对中的第一个节点索引，pairs[:,1]代表所有边对中的第二个节点的索引，所以data1相当于从future_in中按制作的边对图取出所有第一个节点，data2则相当于取出第二个节点。这为后续
                                                                    #计算节点间边的值，即相似度提供了前置的基础。
            
            tree_weight = cosine_distance(data1,data2)              #计算边对节点的相似度
            tree = mst(pairs.repeat(batch_size,1,1),tree_weight,seq_len)   #构建最小生成树
            sorted_index2, sorted_parent2, sorted_child2 = bfs(tree,contex_len)  #使用广度遍历
        else:
