...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        if mmadapter is not None and self.adapter_prompt:
    ...
    def forward(self, x: torch.Tensor, layer_prompt:Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None):
        xls1 = self.ls_1(self.attention(self.ln_1(x), attn_mask=attn_mask))
        x = x + xls1
        # atten_adapter
        if self.mmadapter is not None:
            x = self.mmadapter(x,layer_prompt)
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.resblocks = nn.ModuleList([
            ResidualAttentionBlock(
                width, heads, mlp_ratio, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer,layer_id=layer_id,adapter_prompt=adapter_prompt,mmadapter=mmadapter,mmadapter_aux=mmadapter_aux)
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, final_prompt: Optional[torch.Tensor] = None):
        intermediates = []  # save mid-layer features 
        num_resblocks = len(self.resblocks)
        for i, r in enumerate(self.resblocks):
            if self.grad_checkpointing and not torch.jit.is_scripting():
                x = checkpoint(r, x, attn_mask)
            elif self.adapter_prompt == True:
                layer_prompt = final_prompt[:,i,:,:]
                layer_prompt = layer_prompt.permute(1,0,2)
                x = r(x,layer_prompt=layer_prompt,attn_mask=attn_mask)
            else:
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,viusal_final_prompt: Optional[torch.Tensor] = None):
        ...
        if self.align_before:
            ...
            x = self.transformer(x,final_prompt=viusal_final_prompt)
            x = x.permute(1, 0, 2)
